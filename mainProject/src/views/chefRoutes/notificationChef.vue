<template>
  <div class="container">
    <chefHeader title="Notification" toPage="./accueilChef"/>
      <v-row>
        <v-col>
          <v-text-field
      ...
      <v-row>
        <v-col>
          <div v-for="notif in notifs" :key="notif">
            <h3 class="text-caption text-left"> {{ notif.timeStamp }}</h3>
            <v-alert
      border="left"
      outlined
...
export default {
  ...
  methods:{
    ...
    async getNoitfs(){
      try{
       await api.post('/chefNotif', {id:this.user.id_ChefDep})
      .then(response => {
    ...
    async seeNotifs(){
      try{
       await api.post('/seeChefNotif', {id:this.user.id_ChefDep})
      .then(response => {
